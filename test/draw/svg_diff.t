...
Unlike diff.t, this scene is almost entirely antialiased vector edges (rotate,
star clip, 6x zoom), so even the plain count swings widely between environments
(~1400 px in CI vs ~2600 on a dev box) — too much for an exact coarsened value.
So both counts get the same treatment: print "ok" below a generous threshold and
the /100 count otherwise, so a real regression (a shape dropping out is tens of
thousands of px) still surfaces and fails.

  $ odiff svg_jsoo.png svg_raylib.png 2>&1 | awk '/identical/{print "ok"} /different/{print ($2 < 4000 ? "ok" : int($2/100))}'
  ok
