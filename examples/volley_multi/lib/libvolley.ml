...
(* Same module as [Gamelle.Input_snapshot], minus the io-reading helper — the
...
let read_player_input event ~left ~right ~up ~down =
  {
    left = Input_snapshot.is_pressed event left;
    right = Input_snapshot.is_pressed event right;
    down = Input_snapshot.is_pressed event down;
    jump = Input_snapshot.is_down event up;
  }
